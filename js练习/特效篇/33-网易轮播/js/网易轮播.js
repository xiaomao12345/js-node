window.addEventListener('load', function() {
	...
	var slider_main = myTool.$('slider_main');
	var slider_main_img = slider_main.children;
	var slider_ctl = myTool.$('slider_ctl');
	var iNow = 0;
	...
	//  4.让滚动的内容归位
	/*除第一张外，剩余的图片都定位到盒子右侧，以便滚动，所以j从1开始*/
	...
	for(var i = 0; i < slider_ctl_child.length; i++) {
		// 5.1 监听点击
		slider_ctl_child[i].addEventListener('mousedown', function() {
			if(this.className === "slider-ctl-prev") { //左边
				/*
		         ...
		         3.让这张图片做动画进入
		         */
				myTool.buffer(slider_main_img[iNow], {
					'left': scroll_w
				});
				iNow--;
				if(iNow < 0) {
